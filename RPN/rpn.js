...
function check(input) {
    ...
    for (var i = 0; i < input.length; i++) {
        if (i === 0 && input[i] === '-') {
            ...
            console.log('\n4.You entered an invalid character. Please only enter numbers 0-9 and the following operators/symbols (= - * / % ( ) ^)\n');
            bool = false;
            continue;
        }
        else if (input[i] === '(') {
    ...
    if (lpar.length != rpar.length) {
        console.log('\n5. You entered an invalid expression. Please make sure all open parentheses are closed\n');
        bool = false;
        initCalc();
    }
    return bool;
